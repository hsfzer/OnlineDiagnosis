...
- (void)didMoveToSuperview {
	...
	if ([self.superview isKindOfClass:[UIWindow class]]) {
		//[self setTransformForCurrentOrientation:NO];//window会自动转transform，这一行不要了
	}
}
...
- (void)done {
	isFinished = YES;
	//self.alpha = 0.0f; // 这一行不能要，by copewang
...
- (void)deviceOrientationDidChange:(NSNotification *)notification { 
	...
	if (!superview) {
		...
		//[self setTransformForCurrentOrientation:YES]; //window会自动转transform，这一行不要了
	} else {
