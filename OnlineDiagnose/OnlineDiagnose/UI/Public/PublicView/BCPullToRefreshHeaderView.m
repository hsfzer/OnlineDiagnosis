...
#import "BCPullToRefreshHeaderView.h"


@interface BCPullToRefreshHeaderView ()
...
@implementation BCPullToRefreshHeaderView

+ (BCPullToRefreshHeaderView *)pullToRefreshView
{
    BCPullToRefreshHeaderView *instance = [[BCPullToRefreshHeaderView alloc] initWithFrame:CGRectMake(0, -REFRESH_HEADER_HEIGHT, kScreenWidth, REFRESH_HEADER_HEIGHT)];
    return instance;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
    {
        ...
        self.refreshLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, REFRESH_HEADER_HEIGHT)];
...
{
    if (self.isDragging && scrollView.contentOffset.y < 0)
    {
...
- (void)triggerPullToRefresh:(UIScrollView *)scrollView
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        scrollView.contentInset = UIEdgeInsetsMake(defaultInset.top + REFRESH_HEADER_HEIGHT, defaultInset.left, defaultInset.bottom, defaultInset.right);
